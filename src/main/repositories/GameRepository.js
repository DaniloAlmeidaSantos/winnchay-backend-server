    ...
    async findCalendar(teamId) {
        try {
            const games = await db.raw(
                'SELECT T1.TEAMNAME TEAM1, T2.TEAMNAME TEAM2, G.GAMEDATE, G.GAMERESULT1, G.GAMERESULT2 FROM WINNGAMES G LEFT JOIN WINNTEAMS T1 ON T1.TEAMID = G.GAMEIDTEAM1 LEFT JOIN WINNTEAMS T2 ON T2.TEAMID = G.GAMEIDTEAM2 WHERE T1.TEAMID = ? AND G.GAMEDATE IS NOT NULL',
                [teamId]
            );
    ...
    async detailsGame(teamId1, teamId2) {
        try {
            const game = await db.raw(
                'SELECT T1.TEAMNAME TEAM1, T2.TEAMNAME TEAM2, G.GAMEDATE, G.GAMERESULT1, G.GAMERESULT2, G.GAMEROUND FROM WINNGAMES G LEFT JOIN WINNTEAMS T1 ON T1.TEAMID = G.GAMEIDTEAM1 LEFT JOIN WINNTEAMS T2 ON T2.TEAMID = G.GAMEIDTEAM2  WHERE T1.TEAMID = ? AND T2.TEAMID = ?  AND G.GAMEDATE IS NOT NULL',
                [teamId1, teamId2]
            );
